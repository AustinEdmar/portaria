...
export default function ProfilePage() {
  ...
  return (
    <div style={{ fontFamily: font.body }} className="min-h-screen bg-[#F5F6F8]">
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-slate-200 px-4 sm:px-6 py-3.5 flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          {saved && (
            <span className="text-[12px] text-[#137a58] bg-[#1FA97C]/10 border border-[#1FA97C]/30 rounded-full px-3 py-1">
              Alterações guardadas
            </span>
          )}
          <Button className="bg-[#2452FF] hover:bg-[#1d40cc]" onClick={handleSave}>
            Guardar alterações
          </Button>
        </div>
      </header>
        ...
        <Card className="border-slate-200">
          ...
          <CardContent className="space-y-5">
            ...
            <div className="flex items-start justify-between gap-6">
              ...
              <Switch checked={twoFactor} onCheckedChange={setTwoFactor} className="shrink-0 mt-0.5" />
            </div>
          </CardContent>
        </Card>
